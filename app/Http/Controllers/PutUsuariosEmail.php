...
class PutUsuariosEmail extends Controller
{
    function PutUsuariosEmail(Request $request)
    {

        //Primero valido que el metodo sea PUT
        if ($request->isMethod('put')) {
            
            //Valido que, dentro de la petición, venga un parametro email con valor
            if ($request->has('email')) {
                $email = $request->query('email'); //Para este caso, utilizo request->query, así solo obtengo el email enviado como parametro en la query, no dentro del body
                ...
                if (trim($email) == '') {
                    ...
                    return response()->json(['Error Email' => 'Debe ingresar un valor para realizar la consulta.'], 402);
                }

                //Creo la primera petición para obtener un usuario en base a su email
                ...
                //Guardo la respuesta pasada a json_decode
                $jsonResponse = json_decode($response);

                if (count($jsonResponse) == 0) { //En caso de que no se haya encontrado un usuario con ese email, lanzo un error
                    return response()->json(['Sin resultados' => 'No hubo resultados para ese email'], 404);
                }

                $idUser = $jsonResponse[0]->id; //Obtengo el ID del usuario para luego actualizarlo
                ...
                if (isset($bodyContent->email)) {
                    ...
                    //Si no se cumple el if, agrego el campo email al arreglo como key, y $emailBody como su value
                    $datos['email'] = $emailBody;
                }
                ...
                //Creo una nueva variable para almacenar la respuesta de la API
                ...
                $jsonResponsePut = json_decode($responsePut);

                if (isset($jsonResponsePut->id)) { //Si dentro de la respuesta viene el campo id, es porque se actualizó con éxito
                    $id     = $jsonResponsePut->id; //Id del usuario
                    $name   = $jsonResponsePut->name; //Nombre del usuario
                    $emailR = $jsonResponsePut->email; //Email del usuario 
                    $gender = $jsonResponsePut->gender; //Genero del usuario
                    $status = ($jsonResponsePut->status == 'active') ? true : false; //Estado del usuario pasado a true o false en base a la respuesta
                ...
                } else if ($jsonResponsePut[0]->field == 'email' && $jsonResponsePut[0]->message == 'has already been taken') { //Valido que dentro del arreglo venga un campo email y un campo message = 'has already been taken'
